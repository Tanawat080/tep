<div class="container text-left">    

    <br>
    <div class="row col-sm-12">
        <div class="col-sm-12">
            <h1>ลงทะเบียน Shopping Mall</h1><br><br>
            <p class="indent">ตั้งชื่อร้านและเลือกธีมที่เหมาะสมกับร้านค้าของคุณได้จากตัวเลือกด้านล่างนี้ คุณสามารถคลิกที่ปุ่ม Preview เพื่อดูภาพรวมของธีมนั้นๆได้ (สามารถเปลี่ยนธีมผ่านระบบ Shop Admin ได้ภายหลัง)</p>
            <div class="messageblue">
                <div class="idea">
                    ชื่อร้านค้าจะต้องประกอบไปด้วยตัวอักษรหรือตัวเลขตั้งแต่ 5 ตัวอักษรแต่สูงสุดไม่เกิน 30 ตัวอักษร</div>
            </div><br>
        </div>
        <div class="col-sm-3">
        </div>
        <div class="col-sm-6">
            <div class="input-group">
                <span class="input-group-addon">https://shop.thaiepay.com/</span>
                <input id="shopName" type="text" class="form-control" name="shopName" placeholder="Shop Name">
            </div>
            <br>
            <div class="form-group">
                <?php echo label('password', $this); ?>
                <input id="password" type="password" class="form-control" name="password" placeholder="<?php echo label('password', $this); ?>">
            </div>
            <div class="form-group">
                <?php echo label('rePassword', $this); ?>
                <input id="repassword" type="password" class="form-control" name="repassword" placeholder="<?php echo label('rePassword', $this); ?>">
            </div>
            <select class="form-control" id="sel1">
                <option>แพ็คเกจ / Package</option>
            </select><br>
        </div>
        <div class="col-sm-3">
        </div>
        <br>
        <div class="col-sm-12">
            <div class="border-2">
                <p align="center"><a href="">main_theme</a></p>
                <center>
                    <div class="radio">
                        <label><input type="radio" name="main_theme" checked="checked">main_theme</label>
                    </div>
                    <a href=""><button type="button" class="btn btn-success" name="Preview"  id="Preview" >Preview</button></a>
                </center><br>
            </div><br><center>
                <a href=""><button type="button" class="btn btn-primary" name="Submit"  id="Submit" >Submit</button></a>
        </div>
    </div>
</div>